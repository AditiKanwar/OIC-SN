    ...
    def forward(self, hidden_states, hidden_states2=None, l=None, mix_layer=1000, attention_mask=None, attention_mask2=None):
        ...
        if mix_layer == -1:
            if hidden_states2 is not None:
                ...
                add_noise = 0.4*np.random.beta(2,5)*torch.cuda.FloatTensor(hidden_states.shape).normal_()
                mul_noise = 0.2 * np.random.beta(2,5) * ( 2*torch.cuda.FloatTensor(hidden_states.shape).uniform_()-1) + 1
                #mul_noise = 1
                #add_noise = 0
                hidden_states = mul_noise*hidden_states + add_noise
